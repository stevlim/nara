...
//    }
   ...
   // initMainMenu();
    ...
    $("#btnRegist").on("click", function () {
        ...
        var strToday = $.datepicker.formatDate('yy-mm-dd', new Date());
        ...
        if(typeof isFromDateSetToday != "undefined" && isFromDateSetToday == true){        	
            $("#div_frm input[name='FR_DT']").val(strToday);
        }
        
		if(typeof isMerchantInfoMgmt != "undefined" && isMerchantInfoMgmt == true){
	        gintStepNo = 1;
	        $("#aInsRegistTab1").click();
	        $("#txtInsI_MID").attr("readonly", false);
	        $("input[id^='hidInsI_MID']").val("");
	        $("#hidDupIMID").val("N");
	        $("#hidInsUserAcctFlag").val("Y");
	        $("#div_frm input[name='FR_DT'], #div_frm input[name='CONT_DT'], #div_frm input[name='CO_EXP_TERM']").val(strToday);
        }       
         
        if(typeof isNewContractMgmt != "undefined" && isNewContractMgmt == true){
            fnReset();	
        }
    ...
    var fnClearFormData = function () {
        ...
        $("#div_frm").find("input, select, checkbox, radio").each(function () {
