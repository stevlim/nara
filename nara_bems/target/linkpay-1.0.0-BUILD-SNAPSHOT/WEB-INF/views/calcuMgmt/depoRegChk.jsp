...
var strWorker = "<%=CommonUtils.getSessionInfo(session, "USR_ID")%>";
	...
	function fnSetDDLB() {
		$("select[name=inOutChk]").html(
				"<c:out value='${inOutChk}' escapeXml='false' />");
		$("select[name=CardCompanyList]").html(
				"<c:out value='${CardCompanyList}' escapeXml='false' />");
		$("select[name=typeFlg]").html(
				"<c:out value='${typeFlg}' escapeXml='false' />");
		$("select[name=dateChk]").html(
				"<c:out value='${dateChk}' escapeXml='false' />");
		$("select[name=inOutChkReg]").html(
				"<c:out value='${inOutChkReg}' escapeXml='false' />");
	}
	...
	function fnInquiry() {
		if (typeof objReceiveDeferSearch == "undefined") {
			...
			objReceiveDeferSearch = IONPay.Ajax
					.CreateDataTable(
							"#receiveDeferTable",
							true,
							{
								...
								columns : [
										...
										},
										{
											...
											"render" : function(data) {
												return IONPay.Utils
														.fnStringToDateFormat(data.ACQ_DT)
											}
										},
										{
											"class" : "columnc all",
											...
											"render" : function(data) {
												return IONPay.Utils
														.fnStringToDateFormat(data.DPST_DT)
											}
										},
										{
											"class" : "columnc all",
										...
										},
										{
											...
											"render" : function(data, type, row) {
												if ("00" == row.ST_TYPE
														&& "0" == row.ST_TYPE) {
													return "<input type='button' name='' value='해 제'  onclick=\"javascript:setLayShow(event.x-250,event.y + document.body.scrollTop, '"
															+ row.SEQ
															+ "');\"  class='btn btn-success btn-cons'>";
												} else if ("00" == row.REASON
														&& "1" == row.REASON) {
													return "해제완료";
												} else {
													return "&nbsp;";
										...
										},
										{
											"class" : "columnc all",
											"render" : function(data, type, row) {
												return "<input type='button' name='' value='삭 제'  onclick=\"delData('"
														+ row.SEQ
														+ "');\"  class='btn btn-success btn-cons'>";
											},
											className : "dt-body-center"
	...
	function fnSave() {
		...
		strCallUrl = "/calcuMgmt/calcuCard/receiveDeferRegist.do";
	...
	function selAll(obj_name) {
		for (i = 1; i < 9; i++) {
			...
			$("#receiveDeferRegTable .select2-chosen").text($("#" + obj_name + "_0 option:selected").text());
		}
	}

	function getTIDData(index) {
		arrParameter["tid"] = $("#receiveDeferRegTable #tid_" + index).val();
	...
	function fnTIDDataRet(objJson){
		if (objJson.resultCode == 0) {
			if(objJson.data != null ) {
				...
				if($("#receiveDeferRegTable #mbsNo_" + index).val() != null &&
				   $("#receiveDeferRegTable #mbsNo_" + index).val() != '' &&
				   objJson.data.MBS_NO != $("#receiveDeferRegTable #mbsNo_" + index).val()){
// 					IONPay.Msg.fnAlert('가맹점 번호가 다른 TID입니다. TID를 다시 입력해주세요.');
					$("#receiveDeferRegTable #tid_" + index).val('');
					$("#receiveDeferRegTable #tid_" + index).focus();
				}
				else{
					$("#receiveDeferRegTable #overFlg_" + index).val(objJson.data.OVER_FLG);
					$("#receiveDeferRegTable #overFlgNm_" + index).val(objJson.data.OVER_FLG_NM);
					$("#receiveDeferRegTable #mbsNm_" + index).val(objJson.data.MBS_NO);
					$("#receiveDeferRegTable #amt_" + index).val(objJson.data.GOODS_AMT);
				}
			}
			else{
					...
					<div class="grid-body no-border">
						<form id="frmSearch" name="frmsearch">
							<div class="row form-row">
								<div class="col-md-3">
									<label class="form-label"><spring:message
											code='IMS_BIM_BM_0101' /></label> <select id="dateChk" name="dateChk"
							...
							<div class="row form-row">
								<div class="col-md-3">
									<label class="form-label"><spring:message
											code='IMS_BIM_BM_0179' /></label> <input type="text" id="merNo"
										name="merNo" class="form-control">
								</div>
								<div class="col-md-3">
									<label class="form-label"><spring:message
											code='IMS_PW_DE_12' /></label> <input type="text" id="tId" name="tId"
										class="form-control">
								</div>
							</div>
							<div class="row form-row">
								<div class="col-md-3">
									<label class="form-label"><spring:message
											code='IMS_BIM_BM_0101' /></label> <select id="typeFlg" name="typeFlg"
										class="select2 form-control">
									</select>
								</div>
								<div class="col-md-3">
									<label class="form-label"><spring:message
											code='IMS_BIM_BM_0309' /></label> <select id="inOutChk"
										name="inOutChk" class="select2 form-control">
									</select>
								</div>
								<div class="col-md-3">
									<label class="form-label"><spring:message
											code='IMS_BIM_BM_0178' /></label> <select id="CardCompanyList"
										name="CardCompanyList" class="select2 form-control">
									</select>
								</div>
								<div class="col-md-3">
									<label class="form-label">&nbsp;</label>
									<div>
										...
										<button type="button" id="btnRegist"
											class="btn btn-primary btn-cons">
											<spring:message code='IMS_BM_CM_0062' />
										</button>
									</div>
								</div>
							</div>
						...
						<div id="div_searchResult" style="display: none;">
							<div class="grid simple ">
								<div class="grid-body " id="">
									<table id="receiveDeferTable" class="table" style="width: 100%">
										<thead>
											<tr>
												<th>NO</th>
								...
								<div class="grid simple ">
									<div class="grid-body " id="">
										<div class="col-md-12" style="padding: 0 0 5px 0;">
											<spring:message code='IMS_BIM_BM_0323' />
										</div>
										<div class="col-md-12" style="padding: 0 0 5px 0;">
											<spring:message code='IMS_BIM_BM_0324' />
										</div>
										<table id="receiveDeferRegTable" class="table" style="width: 100%">
											<thead>
												<tr>
													<th><select id="overFlgNm_0" name="inOutChk"
														class="select2 form-control" onChange="selAll('overFlgNm');">
													</select></th>
											...
											<tr>
												<td><select id="overFlgNm_1" name="inOutChk"
													class="select2 form-control">
												</select></td>
											...
											<tr>
												<td><select id="overFlgNm_2" name="inOutChk"
													class="select2 form-control">
												</select></td>
											...
											<tr>
												<td><select id="overFlgNm_3" name="inOutChk"
													class="select2 form-control">
												</select></td>
											...
											<tr>
												<td><select id="overFlgNm_4" name="inOutChk"
													class="select2 form-control">
												</select></td>
											...
											<tr>
												<td><select id="overFlgNm_5" name="inOutChk"
													class="select2 form-control">
												</select></td>
											...
											<tr>
												<td><select id="overFlgNm_6" name="inOutChk"
													class="select2 form-control">
												</select></td>
											...
											<tr>
												<td><select id="overFlgNm_7" name="inOutChk"
													class="select2 form-control">
												</select></td>
											...
											<tr>
												<td><select id="overFlgNm_8" name="inOutChk"
													class="select2 form-control">
												</select></td>
										...
										<center>
											<button type="button" id="btnSave"
												class="btn btn-primary btn-cons">
												<spring:message code='IMS_BIM_BM_0138' />
											</button>
										</center>
									</div>
								</div>
