...
var intSelAuthType;
var intEditAuthNo;
var masterCd = <%= request.getAttribute("MASTER_CD") %>;
var MID = <%= "'"+request.getAttribute("MID")+"'" %>;
...
var fnCreateDataTables = function () {
    ...
    if (typeof objUsrAcctTable == "undefined") {
    	objUsrAcctTable = IONPay.Ajax.CreateDataTable("#tbUsrAcctMgmt", true, {
            url: '/businessMgmt/userAccountMgmt/selectUserAccountMgmtList.do',
            data: function () {
            	var objParam = {};
            	objParam = $("#frmSearchUsrAcct").serializeObject();
            	objParam["MID"] = MID;
                return objParam;
            },
            columns: [
...
function fnCreateWork(data) {
	...
	if(masterCd == '2'){
		strHtml = "<span data-toggle='modal' id='btnEditUserPwd'  data-userid='" + data.USR_ID + "' data-email='" + data.EMAIL + "' style='color:blue; cursor:pointer;'>" + gMessage('IMS_AM_UAM_0005') + "</span>";
		strHtml +="&nbsp&nbsp&nbsp&nbsp";
		strHtml += "<span data-toggle='modal' data-target='#divEditUsrAcct' id='btnUpdateUserAcct' data-userid='" + data.USR_ID + "' data-usertype='" + data.USR_TYPE + "' data-userstatus='" + data.STATUS + "' style='color:blue; cursor:pointer;'>" + gMessage('IMS_AM_UAM_0006') + "</span>";		
	}
...
function fnSelectUserAcctDtlRet(objJson){
	if(objJson.resultCode == 0){
        ...
        $("#CANCLE_AUTH").select2("val", objJson.data[0].CANCLE_AUTH);
        $("#STTLE_AUTH").select2("val", objJson.data[0].STTLE_AUTH);
        $("#MASTER_CD").select2("val", objJson.data[0].MASTER_CD);
        ...
        var strCallUrl  = "/businessMgmt/userAccountMgmt/selectAuthTypeList.do";
...
$(function () {
	...
	$("#txtEditUsrID").on("keyup", function() {
        ...
        if($.trim($("#txtEditUsrID").val()).length >= 4) {
	        ...
	        var strCallUrl   = "/businessMgmt/userAccountMgmt/selectUserID.do";
    ...
    $("#btnEditUserAcct").on("click", function () {
        ...
        /* if(intUserType == 2 && $.trim($("#hidMerchantlist").val()) == "" && $("#divSelectMerchant").is(':hidden') == false){
        	IONPay.Msg.fnAlertWithModal(gMessage('IMS_AM_UAM_0008'), "divEditUsrAcct", true);
            return;
        } */

        if (strModeChk == "") {
            strCallURI = "/businessMgmt/userAccountMgmt/insertUserAccountMgmt.do";
        }
        else {
            strCallURI = "/businessMgmt/userAccountMgmt/updateUserAccountMgmt.do";
        }
    ...
    $("#tbUsrAcctMgmt").on("click", "#btnEditUserPwd", function(){
    	...
    	IONPay.Msg.fnResetPwdConfirm(gMessage('IMS_AM_UAM_0009'), "[ " +strUserName+ " ]", gMessage('IMS_AM_UAM_0035'), strEmail, function () { 
    		...
    		if(regEmail.test($("#inputEmail").val())) {
	    		var strCallUrl  = "/businessMgmt/userAccountMgmt/updateUserPSWD.do";
    ...
    $("#tbUsrAcctMgmt").on("click", "#btnUpdateUserAcct", function(){
    	cleanFrm("frmEditMenu");
        var strUserID   =  $(this).data("userid");
        var strUserType   =  $(this).data("usertype");
        var strUserStatus   =  $(this).data("userstatus");
        var objSettings = $("#frmEditMenu").validate().settings;
        ...
        var strCallUrl  = "/businessMgmt/userAccountMgmt/selectUserAccountDtl.do";
        var strCallBack = "fnSelectUserAcctDtlRet";
        var objParam = {};
        
        objParam["USR_ID"] = strUserID;
        //objParam["USR_TYPE"] = strUserType;
        objParam["USR_STATUS"] = strUserStatus;
        objParam["MID"] = MID;
                                ...
                                <div class="row form-row">
                                    ...
                                    <div class="col-md-3">
                                        ...
                                        <div>
                                            ...
                                            <c:set var="masterCd" value='<%=request.getAttribute("MASTER_CD")%>' />
                                            <c:if test ="${masterCd eq '2' }">
	    ...
	    <div class="modal-content">
	      ...
	      <div class="modal-body">
	           <form name="frmEditMenu" id="frmEditMenu">
                    ...
                    <!-- 권한관리 -->
                    <div class="form-group">
                    	<label class="form-label">취소권한</label>
                      	<span class="help"></span>
                      	<div class="input-with-icon  right">                                       
                          	<i class=""></i>
                          	<select id=CANCLE_AUTH name="CANCLE_AUTH" class="select2 form-control">                              	
                              	<option value="1">취소가능</option>
                              	<option value="2">취소불가</option>
                          	</select>
                      	</div>
                      	
                    	<label class="form-label">정산권한</label>
                      	<span class="help"></span>
                      	<div class="input-with-icon  right">                                       
                          	<i class=""></i>
                          	<select id="STTLE_AUTH" name="STTLE_AUTH" class="select2 form-control">                              	
                              	<option value="1">정산가능</option>
                              	<option value="2">정산불가</option>
                          	</select>
                      	</div>
                    	...
                    	<label class="form-label">마스터여부</label>
                      	...
                      	<div class="input-with-icon  right">                                       
                          	<i class=""></i>
                          	<select id="MASTER_CD" name="MASTER_CD" class="select2 form-control">                              	
            ...
            <div class="modal-footer">
                <div class="pull-right">
                     <button type="button" id="btnEditUserAcct" class="btn btn-danger"><spring:message code="IMS_DM_CPR_0020"></spring:message></button>
                     <button type="button" class="btn btn-default" data-dismiss="modal" onclick="IONPay.Msg.fnResetBodyClass();">
                     	<spring:message code="IMS_RM_TSM_0010"></spring:message>
                     </button>
                </div>
            </div>
	    </div>
