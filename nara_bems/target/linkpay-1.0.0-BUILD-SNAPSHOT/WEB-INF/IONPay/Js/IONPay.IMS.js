...
IONPay.Utils = {
	...
	fnDatePicker : function(strType, intDiff, strID, strBaseID) {
		...
		var intYear  = Number(arrDate[0]);
		var intMonth = Number(arrDate[1]) - 1;
		var intDay   = Number(arrDate[2]);
	...
	fnConvertDateFormat: function (strDate) {
	    ...
	    try {
	        ...
	        strReturnValue = arrValue[0] + arrValue[1] + arrValue[2];		
	    } catch (ex) {
	        strReturnValue = "";
	    }
	...
	fnStringToMonthDateFormat: function (strValue) {
	    ...
	    try {
	        strReturnValue = strValue.substring(0, 4) + "-" + strValue.substring(4, 6);
	    } catch (ex) {
	        strReturnValue = "";
	    }
	...
	/**------------------------------------------------------------
	 * Function Name  : fnStringToPhoneFormat
	 * Description    : -
	 * Author         : yangjeongmo, 2015. 10. 28.
	 * Modify History : Just Created.
	 ------------------------------------------------------------*/
	fnStringToPhoneFormat: function (strValue) {
	    var str = "";

	    try {
	    	 str = strValue.replace(/[^0-9]/g, '');
	    	  var tmp = '';
	    	  if( str.length < 4){
	    	    return str;
	...
	fnStringToLicenseFormat: function (strValue) {
	...
	fnStringToDateFormat: function(strValue) {
	    ...
	    try {
	        if (strValue.length == 8) {
	            strReturnValue = strValue.substring(0, 4) + "-" + strValue.substring(4, 6) + "-" + strValue.substring(6, 8);
	...
	fnStringToDateTimeFormat: function(strValue) {
	    ...
	    try {
	        if (strValue.length == 14) {
	            strReturnValue = strValue.substring(0, 4) + "-" + strValue.substring(4, 6) + "-" + strValue.substring(6, 8) + " " + strValue.substring(8, 10) + ":" + strValue.substring(10, 12) + ":" + strValue.substring(12, 14);
	        }
	    } catch (ex) {
	        strReturnValue = "";
	    }
...
//	    	$("#" + formID + " input[type='text']").val("");
	...
	fnCheckSearchRange: function ($startDT, $endDT, strRange) {
	    ...
	    var startDTYear  = startDT.substring(0, 4);
	    var startDTMonth = Number(startDT.substring(4, 6)) - 1;
	    var startDTDay   = startDT.substring(6, 8);

	    var endDTYear  = endDT.substring(0, 4);
	    var endDTMonth = Number(endDT.substring(4, 6)) - 1;
	    var endDTDay   = endDT.substring(6, 8);
...
$(document).ready(function () {    
    ...
    try {
        $('#MEMO_EDITOR').wysihtml5({"font-styles":  false,
		      "color":        false,
		      "emphasis":     false,
		      "textAlign":    false,
		      "lists":        false,
