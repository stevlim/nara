...
public class MenuRoleMgmtController {
    ...
    public ModelAndView selectUserAccountMgmtList(HttpServletRequest request, @RequestBody String strJsonParameter) throws Exception {
        ...
        try { 
            ...
            objList         = menuRoleMgmtService.selectUserAccountMgmtList(objMap);
            intPageTotalCnt = Integer.parseInt(menuRoleMgmtService.selectUserAccountMgmtListCnt(objMap).toString());
        } catch(Exception ex) {
            intResultCode    = 9999;
            strResultMessage = CommonMessage.MSG_ERR_EXCEPTION;
