 ...
 * Modify History : Just Created.
 ------------------------------------------------------------*/
@Controller
@RequestMapping(value="/baseInfoMgmt/settleReqMgmt")
public class SettleReqMgmtController {
     ...
     * Modify History : Just Created.
     ----------------------------------------------------*/
    @RequestMapping(value = "/settleReqMgmt.do")
    public String settleReqMgmt(HttpServletRequest request, Model model, CommonMap commonMap) throws Exception {
    	List<Map<String,Object>> objList = new ArrayList<Map<String, Object>>();
    	Map< String, Object > objMap = new HashMap<String, Object>();
    	
        model.addAttribute("MENU",               commonMap.get("MENU_GRP_NO"));
        model.addAttribute("SUBMENU",            "13");  
        model.addAttribute("MENU_TITLE",         CommonMessageDic.getMessage(commonMap.get("MENU_GRP_NM").toString()));
        model.addAttribute("MENU_SUBMENU_TITLE", CommonMessageDic.getMessage("IMS_MENU_SUB_0156S"));
        model.addAttribute("AUTH_CD",            commonMap.get("AUTH_CD"));
        model.addAttribute("MENU_GROUP_TYPE",    CommonDDLB.menuType(DDLBType.EDIT));

        model.addAttribute("DateChk",          CommonDDLB.settleReqDateChk());
        model.addAttribute("IdChk",          CommonDDLB.settleReqId());
        model.addAttribute("TypeChk",          CommonDDLB.settleReqType());
        ...
        try
	   	{
        	...
        	String merId = "";
 	   		...
 	   		objList = baseInfoMgmtService.selectMerchantKeyInfo(objMap);
        	
        	model.addAttribute("MID",          objList.get(0).get("MID").toString());
        	model.addAttribute("CO_NM",        objList.get(0).get("CO_NM").toString());
        	model.addAttribute("MER_KEY",      objList.get(0).get("MKEY").toString());
	   	}
	   	catch(Exception e)
	   	{
	   		log.error( "settleReqMgmt-Exception : ", e );
	   	}
        return "/baseInfoMgmt/settleReqMgmt/settleReqMgmt";
    }
    
    //정산요청 리스트 조회
    @RequestMapping(value = "/selectSettleReqList.do", method=RequestMethod.POST)
    public ModelAndView selectSettleReqList(HttpServletRequest request, @RequestBody String strJsonParameter) throws Exception 
    {
    	...
    	log.info( "selectSettleReqList- Start -" );
    	try  
    	{
    		if (!CommonUtils.isNullorEmpty(strJsonParameter)) 
    		{
    			...
    			objList = 	baseInfoMgmtService.selectSettleReqList(objMap);
    			//objList = baseInfoMgmtService
    			
    			intPageTotalCnt = ( Integer ) baseInfoMgmtService.selectSettleReqListTotal(objMap);
    			
    		}
    		else
    		{
    			log.info( "selectSettleReqList-1-파라미터 null " );
    		}
    		
    	}
    	catch(Exception e)
    	{
    		log.error( "selectSettleReqList-Exception : ", e );
    	}
    	finally
    	{
    	...
    	log.info( "selectSettleReqList- End -" );
    	return objMv; 
    }
    
    @RequestMapping(value = "/selectSettleReqListExcel.do", method=RequestMethod.POST)
    public View selectSettleReqListExcel(@RequestBody String strJsonParameter, Map<String, Object> objExcelMap, HttpServletRequest request) throws Exception 
    {
    	ModelAndView objMv  = new ModelAndView();
    	Map< String, Object > objMap = new HashMap<String, Object>();
    	Map< String, Object > dataMap = new HashMap<String, Object>();
    	...
    	try  
    	{
    		if (!CommonUtils.isNullorEmpty(strJsonParameter)) 
    		{
    			...
    			objExcelData = 	baseInfoMgmtService.selectSettleReqList(objMap);
    	...
    	{
    		objExcelMap.put("excelName", "settle_req_mgmt");
            objExcelMap.put("excelData", objExcelData);
            objExcelMap.put("reqData",   objMap);
    	}
    	
    	log.info( "selectSettleReqList- End -" );
    	return new SettleReqMgmtExcelGenerator();
    }
    
    @RequestMapping(value = "/updateSettleReqStatus.do", method=RequestMethod.POST)
    public ModelAndView updateSettleReqStatus(@RequestBody String strJsonParameter) throws Exception
    {
 	   	ModelAndView objMv  = new ModelAndView();
 	   	Map< String, Object > objMap = new HashMap<String, Object>();
 	   	int intResultCode = 0 ;
 	   	String strResultMessage = "";
 	   	log.info( "updateSettleReqStatus update- Start -" );
 	   	try
 	   	{
 	           if (!CommonUtils.isNullorEmpty(strJsonParameter))
 	           {
 		           	log.info( "updateSettleReqStatus update-1-파라미터 null 체크 " );
 		           	objMap = CommonUtils.jsonToMap(strJsonParameter);
 		           	log.info( "updateSettleReqStatus update-1-파라미터 : " + objMap );
 		           	//CommonUtils.initSearchRange(objMap);
 		           baseInfoMgmtService.updateSettleReqStatus(objMap);
 		           //rmApprovalService.updateBlackListClear(objMap);
 	           }
 	           else
 	           {
 	        	   log.info( "updateSettleReqStatus update-1-파라미터 null " );
 	        	   intResultCode = 9999;
 	   	...
 	   	{
 	   		log.error( "updateSettleReqStatus-Exception : ", e );
 	   		intResultCode = 9999;
 	   	...
 	   	finally
         {
 	   	...
 	   	log.info( "updateSettleReqStatus update- End -" );
 	   	objMv.setViewName("jsonView");
 	    return objMv;
    }
    
    @RequestMapping(value = "/updateAllSettleReqStatus.do", method=RequestMethod.POST)
    public ModelAndView updateAllSettleReqStatus(@RequestBody String strJsonParameter) throws Exception
    {
 	   	ModelAndView objMv  = new ModelAndView();
 	   	Map< String, Object > objMap = new HashMap<String, Object>();
 	   	Map< String, Object > objArrayMap = new HashMap<String, Object>();
 	   	int intResultCode = 0 ;
 	   	String strResultMessage = "";
 	   ...
 	   List<Map<String,Object>> list = new ArrayList<Map<String,Object>>();
 	   	...
 	   	try
 	   	{
 	           if (!CommonUtils.isNullorEmpty(strJsonParameter))
 	           {
 		           ...
 		           ArrayList<String> confirmChkProcTid = new ArrayList<>();
 		           confirmChkProcTid = (ArrayList<String>) list.get(0).get("confirmChkProcTid");
 		           
 		          ArrayList<String> confirmChkProcVal = new ArrayList<>();
 		         confirmChkProcVal = (ArrayList<String>) list.get(0).get("confirmChkProcVal");
		           
		           ArrayList<String> confirmChkedTf = new ArrayList<>();
		           confirmChkedTf = (ArrayList<String>) list.get(0).get("confirmChkedTf");
 		           
 		          ArrayList<String> trxStatCdVal = new ArrayList<>();
 		         trxStatCdVal = (ArrayList<String>) list.get(0).get("TRX_STAT_CD_VAL");
	        	   
	        	   for(int i=1; i<confirmChkProcVal.size(); i++) {
	        		  /*log.info( "tid : " + confirmChkProcTid.get(i) );
	        		  log.info( "aaaa2 : " + confirmChkProcVal.get(i) );
	        		  log.info( "aaaa3 : " + confirmChkedTf.get(i) );
	        		  log.info( "aaaa4 : " + trxStatCdVal.get(i) );*/
	        		  Map< String, Object > paramMap = new HashMap<String, Object>();
	        		  
	        		  if(confirmChkProcVal.get(i).equals("Y")) { //"Y" 인것들만 업데이트
	        			  if(confirmChkedTf.get(i).equals("T")) {
	        				  if(trxStatCdVal.get(i).equals("0") || trxStatCdVal.get(i).equals("2")) {
	        					  paramMap.put("TID", confirmChkProcTid.get(i));
	        					  paramMap.put("TRX_STAT_CD", trxStatCdVal.get(i));
	        					  paramMap.put("WORKER", objMap.get("WORKER"));
	        					  
	        					  log.info( "updateSettleReqStatus update-1-파라미터 : " + paramMap );
	        					  baseInfoMgmtService.updateSettleReqStatus(paramMap);
	        				  }
 		          ...
 		          /*  String confirmChkProcTid = ( String ) objMap.get("confirmChkProcTid");	//tid들
 		            String confirmChkProcVal = ( String ) objMap.get("confirmChkProcVal");	//"Y" 인것들만 업데이트
 		            String confirmChkedTf = ( String ) objMap.get("confirmChkedTf");	//confirmChkProcVal = "Y" 이고  confirmChkedTf = "T" 인것들만 업데이트
 		            String trxStatCdVal = ( String ) objMap.get("TRX_STAT_CD_VAL");	//"0"이면 정산승인요청 "2"이면 취소정산요청
   				
	 		       for(int i = 0; i < confirmChkProcVal.length(); i++) {
