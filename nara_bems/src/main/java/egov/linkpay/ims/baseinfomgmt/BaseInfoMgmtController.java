...
public class BaseInfoMgmtController {
    ...
    @RequestMapping(value = "/eromPayDownload.do")
    public void eromPayDownload(HttpServletRequest request, HttpServletResponse response) throws Exception {
    	String id = request.getParameter("id");

        //String fileName = "http://1.227.122.150:8083/download/testFile.dll";
    	String fileName = "WEB-INF/download/eromPayDll.dll";
        String fileType = "";
        ...
        response.setHeader("Content-disposition","attachment; filename=eromPayDll.dll");
        //response.setHeader("size","attachment; filename=eromPayDll.dll");
        
        // Assume file name is retrieved from database
        // For example D:\\file\\test.pdf

        //String path = BaseInfoMgmtController.class.getResource("").getPath();
        String path = request.getSession().getServletContext().getRealPath("/");	
        
        //String path = request.getSession().getServletContext().getRealPath("/download/testFile.dll");
        
        File my_file = new File(path + fileName);
        response.setContentLength((int)my_file.length());  //파일크기를 브라우저에 알려준다.
