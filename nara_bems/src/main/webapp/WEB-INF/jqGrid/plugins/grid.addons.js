...
// This is the ols search Filter method used in navigator.
	...
	filterGrid : function(gridid,p){
		...
		return this.each(function(){
			var self = this;
			this.p = p;
			if(this.p.filterModel.length === 0 && this.p.gridModel===false) { IONPay.Msg.fnAlert("No filter is set"); return;}
			if( !gridid) {IONPay.Msg.fnAlert("No target grid is set!"); return;}
			...
			if(gcolMod) {
				...
				IONPay.Msg.fnAlert("Could not get grid colModel"); return;
			}
