...
$(document).mouseup(function (e){
	var container = $(".menu-area2");
	
	if($("#main-menu").has(e.target).length === 0 && $(".menu-area2").has(e.target).length === 0) {
		
		$(".menu-area2").data("current", "0");
		$(".menu-area2").css({
			"opacity": "0",
			"visibility": "hidden"
		});
...
<!--                 		        <b><span style="font-size:20pt;color:coral">EROM</span></b> <br/>  -->
<!--                 		        <b><span style="font-size:20pt;color:coral">PAY</span></b> -->
                			<img src="//<spring:eval expression="@config['SERVICE_DOMAIN']"/>/Bootstrap/assets/img/erompay_logo_lnb_54x63.png" alt="" />
                		</a>
                	</li>
                    ...
                    <li id="menu_li_dashboard" class='sidenav'>
	                    <a href="/home/dashboard/dashboard.do">
		                    <i class="fa fa-tachometer" id="<c:out value="menu_i_dashboard"/>"></i>
                           ...
                           <c:when test="${menu.DEPTH eq '1'}">
                               ...
                               <c:choose>
                               	   <c:when test="${not status.last and (menuList[status.index+1].DEPTH eq menu.DEPTH)}">
                               	   		<c:if test="${depth == 2}">
                               	        	<c:set var="cls" value="fa fa-clone" />
	                            		</c:if>
	                            		<c:if test="${depth == 3}">
	                            			<c:set var="cls" value="fa fa-calendar" />
	                            		</c:if>
	                            		<c:if test="${depth == 4}">
	                            			<c:set var="cls" value="fa fa-address-book-o" />
	                            		</c:if>
	                            		<c:if test="${depth == 5}">
	                            			<c:set var="cls" value="fa fa-users" />
	                            		</c:if>
	                            		<c:if test="${depth == 6}">
	                            			<c:set var="cls" value="fa fa-desktop" />
	                            		</c:if>
	                            		<c:if test="${depth == 7}">
	                            			<c:set var="cls" value="fa fa-line-chart" />
	                            		</c:if>
	                            		<c:if test="${depth == 8}">
	                            			<c:set var="cls" value="fa fa-cog" />
	                            		</c:if>
	                            		<c:if test="${depth == 9}">
	                            			<c:set var="cls" value="fa fa-users" />
	                            		</c:if>
	                            		<c:set var="depth" value="${depth+1 }" />
                               <li id="<c:out value="menu_li_${menu.DEPTH}_${status.index}"/>" class='sidenav' onclick="fnClickMenuDepth1(this); return false;"><a href="#"><i class="${cls}" id="<c:out value="menu_i_${menu.DEPTH}_${status.index}"/>"></i><span class="title"><spring:message code='${menu.MENU_NM }'/></span> <span id="<c:out value="menu_span_${menu.DEPTH}_${status.index}"/>"></span></a>
                               </li>
                               	   </c:when>
        ...
        <div id="depth2">
	        <ul class='menu-area2' id="authority" data-current="0"></ul>
	        <ul class='menu-area2' id="calcu" data-current="0"></ul>
	        <ul class='menu-area2' id="oper" data-current="0"></ul>
	        <ul class='menu-area2' id="business" data-current="0"></ul>
	        <ul class='menu-area2' id="payment" data-current="0"></ul>
	        <ul class='menu-area2' id="baseinfo" data-current="0"></ul>
	        <ul class='menu-area2' id="rm-approval" data-current="0"></ul>
	        <ul class='menu-area2' id="total" data-current="0"></ul>
	        <ul class='menu-area2' id="message" data-current="0"></ul>
        </div>
